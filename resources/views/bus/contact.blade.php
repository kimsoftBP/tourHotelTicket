	...
	<form action="{{route('bus.customer.message.post',app()->getLocale())}}" method="POST">
		<div class="col-lg-12">
			...
			<div class="form-group">
				...
				<textarea class="form-control @error('text') is-invalid @enderror" name="text" id="text">{{old('text',__('messages.contactMessage',['from'=>$data['messageFrom'] ,'fromdate'=>$data['fromdate'] ,'todate'=>$data['todate'] , 'people'=>$data['persons']]))}}</textarea>
				@error('text')
					<div class="text-danger">{{$message}}</div>
				@enderror
			</div>
