			...
			@if($data['restaurants']!=NULL)
				@foreach($data['restaurants'] as $row)
					<tr>
						...
						<td>{{$row->city}}</td>
						<td>
							<a class="btn btn-sm btn-primary" href="{{route('restaurant.customer.message',['locale'=>app()->getLocale(),'from'=>$data['search']['from'],'comp'=>$row->id,'persons'=>$data['search']['persons'] ])}}">
