...
class PartnerRestaurantSubpageController extends Controller
{
    // 
        //
    public function index(Request $req){
        $user=Auth::user();
        $data['buscompany']=$user->RestaurantCompanyPermission->first()->RestaurantCompany;    
        $data['country']=$data['buscompany']->country;
        $data['region']=$data['country']->region;    
        $data['subpage']=$data['buscompany']->Subpage->first();
        return view('partner.restaurant.subpage')->with('data',$data);
    }
    public function postEdit(Request $req){
        $validateDate=$req->validate([            
            'hotelcomp'=>'required|exists:App\RestaurantCompany,id',
        ...
        $subpage=Subpage::updateOrCreate([
            'id'=>$req->subpage??'',
            'restaurant_companyid'=>$req->hotelcomp,
            ],[
                'title'=>$req->title,
                'text'=>$req->text,
